...
class FluxGeneratorClass:
    ...
    def __init__(self, Table, LogTable=True, LogAxis=['tau']):
        ...
        '''
        self._Interpolator = InterpolatorClass.InterpolatorClass(Table, 
            LogTable=LogTable, LogAxis=LogAxis)
    ...
    def GetTransformedValueSariAdiabatic(self, Taus, P):
        f0 = (P['n']/P['E'])**(1./3.)/(1.+P['z'])
        Times = Taus / f0

        nu_c = 2.7e12 * P['epsb']**(-1.5) * (P['E']*0.01)**(-0.5) * P['n']**(-1.0) * (Times/86400.)**(-0.5)
        nu_m = 5.7e14 * P['epsb']**(0.5) * P['epse']**2.0 * (P['E']*0.01)**(0.5) * (Times/86400.)**(-1.5)
        F_peak = [6.5273222e+12 * P['epsb']**(0.5) * (P['E']*0.01) * P['n']**(0.5)]*len(Times)

        return F_peak, nu_c, nu_m

    def GetTransformedValueSariRadiative(self, Taus, P):
        f0 = (P['n']/P['E'])**(1./3.)/(1.+P['z'])
        Times = Taus / f0

        nu_c = 1.3e13 * P['epsb']**(-1.5) * (P['E']*0.01)**(-4./7.) * P['GammaB']**(4./7.) * P['n']**(-13./14.) * (Times/86400.)**(-2./7.)
        nu_m = 1.2e14 * P['epsb']**(0.5) * P['epse']**2.0 * (P['E']*0.01)**(4./7.) * P['GammaB']**(4./7.) * P['n']**(-1./14.) * (Times/86400.)**(-12./7.)
        F_peak = 912793438600 * P['epsb']**(0.5) * (P['E']*0.01)**(8./7.) * P['GammaB']**(-8./7.) * P['n']**(5./14.) * (Times/86400.)**(-3./7.)
    ...
    def GetSpectral(self, Times, Freqs, P):
        ...
        if np.isnan(F_peak[0]):
            ...
            IdxSlow1 = IdxSlow * (Freqs < 0.)
            IdxSlow3 = IdxSlow * (Freqs > 0.)
            IdxSlow2 = IdxSlow * (Freqs < 0.)
            IdxFast1 = (~IdxSlow) * (Freqs < nu_c)
            IdxFast2 = (~IdxSlow) * (Freqs >= nu_c) * (Freqs < nu_m)
            IdxFast3 = (~IdxSlow) * (Freqs >= nu_m)
            #IdxFast1 = (~IdxSlow) * (Freqs < 0.)
            #IdxFast3 = (~IdxSlow) * (Freqs > 0.)
            #IdxFast2 = (~IdxSlow) * (Freqs < 0.)

            #print(np.where(IdxSlow1))
            #print(np.where(IdxSlow2))
            #print(np.where(IdxSlow3))
            #print(np.where(IdxFast1))
            #print(np.where(IdxFast2))
            #print(np.where(IdxFast3))

            inu_m = 1.0/nu_m
            inu_c = 1.0/nu_c

            p = P['p']
            output = np.zeros(len(Times), dtype=np.longdouble)
